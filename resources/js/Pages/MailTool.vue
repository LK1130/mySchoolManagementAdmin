        ...
        <div class="w-full h-auto p-8 relative bg-secondaryBackground rounded-b-xl flex flex-col items-center">
            ...
            <div class="p-4 w-full  rounded-xl">
                <table class="text-white w-full rounded-lg ">
                    ...
                    <tbody class="text-sm mt-3">
                        <tr class="border-b" v-for="result in props.mails.data" :key="result">
                            <!-- <td>{{ result.p_title }}</td> -->
                            <td class="text-left py-4">{{ result.m_title }}</td>
                            <td class="text-center py-4">{{ result.m_description.substring(0, 20) + "..." }}</td>
                            <td class="text-center py-4">{{ moment(result.created_at).calendar() }}</td>
                            <td v-if="result.m_category == 1" class="text-center py-4 text-blueTextColor">
                                Info
                            </td>
                            <td v-else-if="result.m_category == 2" class="text-center py-4 text-yellowTextColor">
                                Direct Message
                            </td>
                            <td v-else-if="result.m_category == 3" class="text-center py-4 text-tertiaryBackground">
                                Alert
                            </td>
                        </tr>
                    </tbody>
                ...
                <div class="flex justify-center items-center my-10">
                    <Pagination class="z-10" :links="mails.links">
                    </Pagination>
                </div>
            </div>
            ...
            <div class="flex flex-col mt-10 md:flex-row w-full px-5 items-center justify-center text-white">
                <div
                    ...
                    <Link :href="route('mailtool.create')" class="flex flex-row justify-center items-center space-x-3">
