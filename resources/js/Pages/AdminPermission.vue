        ...
        <form @submit.prevent="submit">
            <div class="w-full h-auto p-8 relative bg-secondaryBackground rounded-xl flex flex-col space-y-5">
                <div class="w-full flex flex-start border border-white rounded-md overflow-hidden"
                    v-for="role in props.role_page" :key="role.id">
                    <div class="flex items-center justify-center border-r border-white py-5 w-32">
                        <p class="text-white text-xl">
                            {{ role.r_name }}
                        </p>
                    </div>
                    <div class="flex items-center px-7 py-2" v-for="page in props.pages" :key="page">
                        <div class="flex flex-col items-center mx-5">
                            <p class="text-white">{{ page.p_name }}</p>
                            <input type="checkbox" class="mt-1" checked v-if="role.page.find(obj => obj.id ===  page.id)" />
                            <input type="checkbox" class="mt-1" v-else />
                        </div>
                    </div>
                </div>
